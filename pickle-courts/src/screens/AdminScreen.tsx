...
export default function AdminScreen() {
  ...
  // ====== DROPDOWN Venue -> Court (Dialog Picker) ======
  ...
  const filters = useMemo(
    () => ({ courtId: courtId || undefined, date: date || undefined }),
    [courtId, date]
  );
    ...
    <View style={{ flex: 1, backgroundColor: "#F9FAFB" }}>
      <View style={{ flex: 1, padding: 16, gap: 14 }}>
          ...
          <Card.Content style={{ gap: 12 }}>
            ...
            <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
              <Chip
                ...
                style={{ backgroundColor: date === dayjs().format("YYYY-MM-DD") ? "#E0E7FF" : "#F3F4F6" }}
                textStyle={{ color: date === dayjs().format("YYYY-MM-DD") ? "#4338CA" : "#374151", fontWeight: "600" }}
              >
                Hôm nay
              </Chip>
              <Chip
                icon="calendar"
                selected={date === dayjs().add(1, "day").format("YYYY-MM-DD")}
                onPress={() => setDate(dayjs().add(1, "day").format("YYYY-MM-DD"))}
                style={{ backgroundColor: date === dayjs().add(1, "day").format("YYYY-MM-DD") ? "#E0E7FF" : "#F3F4F6" }}
                textStyle={{ color: date === dayjs().add(1, "day").format("YYYY-MM-DD") ? "#4338CA" : "#374151", fontWeight: "600" }}
              >
                Ngày mai
              </Chip>
              <Chip icon="close" onPress={() => setDate("")} style={{ backgroundColor: "#F3F4F6" }} textStyle={{ color: "#374151", fontWeight: "600" }}>
                Xoá ngày
              </Chip>
            </View>
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              <Button mode="contained" icon="filter" onPress={() => q.refetch()} loading={q.isFetching} style={{ borderRadius: 12 }}>
                Lọc
              </Button>
              <Button
                mode="outlined"
        ...
        <Card style={{ backgroundColor: "#fff", borderRadius: 16, borderColor: "#E5E7EB", borderWidth: 1 }}>
          <Card.Content style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 4 }}>
            ...
            <View>
              ...
              <Text style={{ color: "#16A34A", fontWeight: "700", fontSize: 18 }}>{summary.revenue.toLocaleString()} đ</Text>
            </View>
          </Card.Content>
        </Card>
        ...
        {q.isLoading ? (
          ...
          <Text style={{ paddingTop: 16, color: "#EF4444" }}>Không tải được danh sách</Text>
        ) : (
          <FlatList
            data={sortedData}
            ...
            ListEmptyComponent={<Text style={{ color: "#9CA3AF", textAlign: "center", marginTop: 16 }}>Không có booking nào</Text>}
            renderItem={({ item }) => {
        ...
        <Dialog visible={venueDlgOpen} onDismiss={() => setVenueDlgOpen(false)}>
          ...
          <Dialog.ScrollArea style={{ maxHeight: 340 }}>
            <ScrollView>
              ...
              {!venueList?.length && (
                <List.Item title="Chưa có địa điểm" description="Tạo venue trước" />
              )}
            </ScrollView>
          </Dialog.ScrollArea>
          <Dialog.Actions>
